...
def main():
    ...
    #Investigate the gateway log
    #filtered_records, _ = filter_log_by_regex(log_file, 'sshd', print_summary=True, print_records=True)
    #filtered_records, _ = filter_log_by_regex(log_file, 'invalid user', print_summary=True, print_records=True)
    #filtered_records, _ = filter_log_by_regex(log_file, 'invalid user.*220.295.35.40', print_summary=True, print_records=True)
    #filtered_records, _ = filter_log_by_regex(log_file, 'error', print_summary=True, print_records=True)
    ...
    # Extract data from the gateway log
...
def get_file_path_from_cmd_line(param_num=1):
    ...
    if len(sys.argv) < param_num + 1:
        print(f'Error: Missing log file pth expected as command line parameter {param_num}.')
        sys.exit('Script execution aborted')

    # Get the parameter valuse and convert it to  full path
